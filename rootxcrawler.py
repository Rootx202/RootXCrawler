...
from core.colors import good, info, run, green, red, white, end,cyan, bad

# RootXCrawler banner with the new project name
...
%s╔══════════════════════════════════════════════════════════════╗
║  >> RootXCrawler: Crawling deep. Seeing everything. <<       ║
║  Author: rootx | Version: 1.0 | License: MIT                 ║
╚══════════════════════════════════════════════════════════════╝%s
'''

print(banner % (red, cyan, end))
try:
    from urllib.parse import urlparse  # For Python 3
except ImportError:
    print('%s RootXCrawler runs only on Python 3.2 and above.' % info)
    quit()
...
Examples:
  ...
  Regex filter and exclude patterns:
    python rootxcrawler.py -u https://example.com -r "\\.php" --exclude "\\.css|\\.png"

  Full-featured crawl:
    python rootxcrawler.py -u https://example.com -l 5 -t 20 -e csv --dns --keys --wayback

  Clone website with headers:
...
""")
# Options
parser.add_argument('-u', '--url', help='Root URL to begin crawling from (e.g. https://example.com)', dest='root')
parser.add_argument('-c', '--cookie', help='HTTP cookie header value for authenticated requests (e.g. "sessionid=abc123")', dest='cook')
parser.add_argument('-r', '--regex', help='Regular expression to filter or match URLs during crawling (e.g. "\\.php|\\.asp")', dest='regex')
parser.add_argument('-e', '--export', help='Output format for exported results (e.g. -e json)', dest='export', choices=['csv', 'json', 'html', 'sqlite'])
parser.add_argument('-o', '--output', help='Directory where result files should be saved (e.g. -o ./results)', dest='output')
parser.add_argument('-l', '--level', help='Maximum crawl depth from the root URL (default: unlimited, e.g. -l 3)', dest='level',
                    type=int)
parser.add_argument('-t', '--threads', help='Number of concurrent worker threads to use (default: 10, e.g. -t 20)', dest='threads',
                    type=int)
parser.add_argument('-d', '--delay', help='Delay in seconds between HTTP requests (e.g. -d 0.5)',
                    dest='delay', type=float)
parser.add_argument('-v', '--verbose', help='Enable detailed logging output (e.g. -v)', dest='verbose',
                    action='store_true')
parser.add_argument('-s', '--seeds', help='Additional seed URLs to start crawling from (e.g. -s https://a.com https://b.com)', dest='seeds',
                    nargs="+", default=[])
parser.add_argument('--stdout', help='Print selected output variables to stdout (e.g. --stdout url,status,size)', dest='std')
parser.add_argument('--user-agent', help='Custom User-Agent header value(s). Separate multiple agents with || for rotation (e.g. --user-agent "Mozilla/5.0")',
                    dest='user_agent')
parser.add_argument('--exclude', help='Exclude URLs that match this regular expression (e.g. --exclude "\\.css|\\.js|\\.png")',
                    dest='exclude')
parser.add_argument('--timeout', help='HTTP request timeout in seconds (default: 10, e.g. --timeout 30)', dest='timeout',
                    type=float)
parser.add_argument('-p', '--proxy', help='Proxy server in IP:PORT or DOMAIN:PORT format (e.g. -p 127.0.0.1:8080)', dest='proxies',
                    type=proxy_type)
parser.add_argument('--config', help='Load configuration from a JSON file (e.g. --config myconfig.json)', dest='config')

# Switches
parser.add_argument('--clone', help='Clone the website content locally for offline analysis (e.g. --clone)',
                    dest='clone',
                    action='store_true')
parser.add_argument('--headers', help='Include HTTP headers in requests and output (e.g. --headers)', dest='headers',
                    action='store_true')
parser.add_argument('--dns', help='Enumerate subdomains and gather DNS information (e.g. --dns)',
                    dest='dns', action='store_true')
parser.add_argument('--keys', help='Scan page content for exposed secret keys and credentials (e.g. --keys)', dest='api',
                    action='store_true')
parser.add_argument('--update', help='Update RootXCrawler to the latest version (e.g. --update)', dest='update',
                    action='store_true')
parser.add_argument('--only-urls', help='Only extract URLs without additional content analysis (e.g. --only-urls)', dest='only_urls',
                    action='store_true')
parser.add_argument('--wayback', help='Use archive.org to fetch historical URLs as crawl seeds (e.g. --wayback)',
...
if args.config:
    try:
        ...
        print('%s Could not load config file: %s' % (bad, e))
        quit()

    valid_destinations = [action.dest for action in parser._actions]
    config_defaults = {
        key: value for key, value in config_defaults.items()
...
failed = set()  # URLs that rootxcrawler failed to crawl
